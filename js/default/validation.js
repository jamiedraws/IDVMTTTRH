	...
	if ("app" in global) {
		...
		validator.addProperties({
			...
			validateForm: function (e) {
				...
				let scrollEle;
				let billingDifferentThanShipping = $("[name=ShippingIsDifferentThanBilling]").is(":checked");

				if (!this.elementExists(card) ||(this.elementExists(card) && card.checked)) {
		...
		function validate(e) {
			validator.validateForm.call(validator, e);
        }
		...
		addEventListener("AVS_Complete", function () {
			var forms = validator.getForms();
            for (var i = 0; i < forms.length; i++) {
				forms[i].removeEventListener("click", validate);
			}
